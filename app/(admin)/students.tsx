      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalCard}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>{initial?.name ? 'Edit Student' : 'Add Student'}</Text>
          ...
          <View style={styles.classRow}>
            {CLASSES.map((c) => (
              <TouchableOpacity
                key={c}
                style={[styles.classChip, cls === c && styles.classChipActive]}
                onPress={() => setCls(c)}
              >
                <Text style={[styles.classChipText, cls === c && styles.classChipTextActive]}>{c}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <TouchableOpacity
            ...
            onPress={() => onSave({ name, email, password, class: cls })}
            disabled={loading}
          >
            {loading ? <ActivityIndicator color={Colors.white} /> : <Text style={styles.saveBtnText}>Save Student</Text>}
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
...
export default function StudentsScreen() {
  ...
  const createMutation = useMutation({
    mutationFn: (d: UserFormData) =>
      apiRequest('POST', '/api/users', { ...d, role: 'student' }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, d }: { id: string; d: UserFormData }) => {
      const body: Record<string, string> = { name: d.name, email: d.email, class: d.class };
  ...
  const handleSave = async (d: UserFormData) => {
  ...
  const handleDelete = (u: AppUser) => {
    Alert.alert('Delete Student', `Remove ${u.name} from the system?`, [
...
const styles = StyleSheet.create({
  ...
  classPillText: { fontFamily: 'Inter_500Medium', fontSize: 11, color: Colors.accent },
  ...
  classRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 20 },
  classChip: {
    paddingHorizontal: 14,
    paddingVertical: 7,
    borderRadius: 20,
  ...
  classChipActive: { backgroundColor: Colors.accent, borderColor: Colors.accent },
